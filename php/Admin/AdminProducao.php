...
if (@$_GET['func'] == 'true') {
    ...
    if ($_SESSION['status_pedido'] == 1) {

        $query = "UPDATE pedidos SET status_pedido = 2 WHERE id = '$id' ";
        $action = mysqli_query($conn, $query);
    } else if ($_SESSION['status_pedido'] > 1) {

        echo "<script>alert('O Pedido ja foi enviado')</script>";
    }
}
    ...
    <div class="bar-lateral">
        <div class="content-bar">
            ...
            <div class="escolhas-bar">
                <div class="sch">
                    <a href="AdminPedidos.php" target="_blank"><i class="fas fa-mail-bulk" style="font-size: 20px;"></i><span>
                            PEDIDOS</span></a>
                </div>
                <div class="sch">
                ...
                <div class="sch">
                    <a href="AdminBackup.php" target="_blank"><span><i class="fas fa-database" style="font-size: 20px;"></i> BACKUP</span></a>
                </div>


                <div class="sch">
                    <a href="../logout.php" target="_blank"><i class="fas fa-sign-out-alt" style="font-size: 20px;"></i><span>
                            SAIR</span></a>
                </div>
            </div>
        </div>
                    ...
                    <div class="bg-progress">
                        ...
                        progressbar" style="width: 25%" aria-valuenow="15" aria-valuemin="0" aria-valuemax="100">
                            ...
                            <div class="progress-bar <?php if ($pedido == 2 || $pedido == 3 || $pedido == 4) {
                                                            ...
                                                            echo 'desativado';
                                                        } ?>" role=" progressbar" style="width: 25%" aria-valuenow="30" aria-valuemin="0" aria-valuemax="100">
                            ...
                            <div class="progress-bar bg-info <?php if ($pedido == 3 || $pedido == 4) {
                                                                    ...
                                                                    echo 'desativado';
                                                                } ?>" role="progressbar" style="width: 25%" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100">
                            ...
                            <div class="progress-bar bg-success <?php if ($pedido == 4) {
                                                                    ...
                                                                    echo 'desativado';
                                                                } ?>" role="progressbar" style="width: 25%" aria-valuenow="15" aria-valuemin="0" aria-valuemax="100">
                        ...
                        <div class="group-btn">
                            <hr>
                            <button class="btn btnall btnwar" onclick="window.location.href='AdminPedidos.php'">VOLTAR</button>
                            <button class="btn btnall btndwd" onclick="window.location.href='AdminDown.php'">BAIXAR
                                ARQUIVO</button>
                        ...
                        <div class="responsavel-inpt">
                            <form method="get">
                                ...
                                <input type="text" name="responsavel" <?php if ($resp != null) {
                                                                            echo ' disabled="" ';
                                                                        } ?> placeholder="Digite seu nome" maxlength="50" 
                                                                        value="<?php if ($resp != null) {
                                                                                                                                        echo $resp;
                                                                                                                                    } else {
                                                                                                                                        echo '';
                                                                                                                                    } ?>" required>
                                ...
                                ?>
                                    <button type="submit" class="btn btnall btnnew" onclick="window.location.href = 'AdminProducao.php?func=puxar&id=<?php echo $id ?>' ">ENVIAR</button></a>
                                ...
                                if ($_SESSION['status_pedido'] == 1) {
                                    echo "<a href='AdminProducao.php?func=true&' ";
                                    echo $_SESSION['id_pedido'], " '> <button class='btn-success btn btnall'>INICIAR PRODUÇÃO</button> </a>";
                                } ?>
                            </form>
                        </div>
                    </div>
...
/**
 * Por algum motivo o disable afetou o funcionamento
 * do pedido == 3 impedindo a query de avancar 
 * fazer tratativa de erro
 */


if (@$_GET['responsavel'] != null || @$_GET['func'] == 'puxar') {
    $id = $_SESSION['id_pedido'];
    $responsavel = $_GET['responsavel'];

    // $responsavel = filter_input(INPUT_GET, 'responsavel', FILTER_SANITIZE_STRING);
    // $query = "INSERT INTO pedidos responsavel VALUE '$responsavel' WHERE id = '$id' ";
    // echo "<script> alert(' {$responsavel} e {$id} e {$pedido} '); </script>";

    if ($pedido == 1) {
        $query = "UPDATE pedidos SET responsavel = '$responsavel' WHERE id = '$id' ";
        $action = mysqli_query($conn, $query);

        echo "<script>location.href=\"AdminProducao.php?func=true\";</script>";
    }

    if ($pedido == 2) {
        // Arrumar a hora da produção, SET hora_producao =

        // $query = "INSERT INTO pedidos responsavel VALUE '$responsavel' WHERE id = '$id' ";
        // SET time_pedido 'CURRENT_TIMESTAMP'
        // $query = "UPDATE pedidos SET responsavel = '$responsavel' WHERE id = '$id' ";

        /* $query = "UPDATE pedidos SET responsavel = '$responsavel' WHERE id = '$id' ";
        $action = mysqli_query($conn, $query); */

        $query = "UPDATE pedidos SET status_pedido = 3 WHERE id = '$id' ";
        $action = mysqli_query($conn, $query);

        if ($action) {
            echo "<script>alert('Pedido enviado com sucesso !');location.href=\"AdminProducao.php\";</script>";
        } else {
        }
    }
} else {
