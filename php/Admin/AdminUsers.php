...
<head>
    ...
    <link rel="stylesheet" href="../../css/modal-css/modal-css.css">
</head>
    ...
    <div class="body">
        ...
        <div class="conteudo-body">
            <div class="btn-newuser">
                ...
                <form method="get">
                    <input id="assunto" type="text" class="form-control" name="pesquisar" placeholder="Fazer uma Busca">
                    <br>
            ...
            <div class="bg-table">
                <table class="table table-responsive table-action">
                    ...
                    $query = "SELECT id, nome_usuario, senha_usuario, nivel_usuario FROM usuarios LIMIT $inicio, $quantidade";
...
<?php
        ...
        if(isset($_POST['editar'])){
            // $id = $_POST['numero_id'];
            $nome_inp = $_POST['nome'];
            $nome_inp = preg_replace('/[^[:alnum:]_]/', '', $nome_inp);
            
            $senha_inp = $_POST['senha'];
            $senha_inp = preg_replace('/[^[:alnum:]_]/', '', $senha_inp);
...
<?php
    if(isset($_POST['adicionar'])){
        ...
        $nome_inpt = preg_replace('/[^[:alnum:]_]/', '', $nome_inpt);
        
        $senha_inpt = $_POST['senha'];
        $senha_inpt = preg_replace('/[^[:alnum:]_]/', '', $senha_inpt);
        ...
        // echo "<script>window.alert('{$nome_inpt}')</script>";
        ...
        if($nome != $nome_inpt){
            ...
            // echo "<script>window.alert('{$aciona}')</script>";
            
            if($aciona > 0){
                echo "<script>window.alert('Nome de Usuario Ja Existente')</script>";         
            } else {
        
                $query = "INSERT INTO usuarios (nome_usuario, senha_usuario, nivel_usuario) VALUES ('$nome_inpt', '$senha_inpt', '$nivel_inp') ";
